...
| **Ni foam or other high-surface-area substrate** – loading often reported per geometric area of the foam, but the foam itself contributes to OER current. | Record the loading as stated, but add a flag in `notes` (“Ni foam background may contribute”). The primary `overpotential_eta10_mV` is taken as reported, but if a background-corrected value is available, it can be stored in a separate field (currently not in schema, use `notes`). |
...
| **Faradaic efficiency not measured but oxygen evolution assumed** | `faradaic_efficiency_percent` is left empty; no assumption is made. The catalyst is still valid for OER metrics as long as the paper claims OER and provides electrochemical data. |
